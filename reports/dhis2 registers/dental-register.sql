SELECT `HUDHURIO LA KWANZA`,(@row_number:=@row_number+1) AS "Na",TAREHE,`NAMBA YA HUDHURIO`,`JINA LA MGONJWA`,`MAHALI ANAISHI`,UMRI,`JINSIA YA MGONJWA`,`UZITO(kg)`,`UREFU(cm)`,`VIPIMO`,`MATOKEO YA VIPIMO`,DIAGNOSIS,`MATIBABU`,`MATOKEO YA MAHUDHURIO`,`MALIPO` FROM(
SELECT
	...
	WHERE v.location_id=44 AND v.voided=0 AND CAST(CONVERT_TZ(v.date_started,'Etc/GMT+3','GMT') AS DATE) BETWEEN '2022-11-01' AND '2022-11-30'
