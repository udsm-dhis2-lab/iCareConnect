...
from (select  	SUM(IF(MONTH(CONVERT_TZ(v.date_started,'Etc/GMT+3','GMT')) = 1, 1, 0)) AS col1,
      ...
      from visit v
        ...
        INNER JOIN lb_sample sp ON sp.sample_id = so.sample_id AND sp.sample_id IN(SELECT sps.sample_id FROM lb_sample_status sps WHERE sps.category = 'HAS_RESULTS') AND sp.sample_id NOT IN(SELECT sps.sample_id FROM lb_sample_status sps WHERE sps.category = 'RESULT_AUTHORIZATION')
