...
(SELECT GROUP_CONCAT( DISTINCT CASE WHEN spstatus.status = 'HAS RESULTS' THEN ROUND((UNIX_TIMESTAMP(spstatus.timestamp)- UNIX_TIMESTAMP(sp.date_time) )/60,0) ELSE NULL END)
        FROM lb_sample_status spstatus
        WHERE spstatus.sample_id = sp.sample_id
    ) AS requested_full_result,
(SELECT GROUP_CONCAT( DISTINCT ROUND((UNIX_TIMESTAMP(spstatus.timestamp)- UNIX_TIMESTAMP(sp.date_time) )/60,0) )
        FROM lb_sample_status spstatus
        INNER JOIN lb_sample_order so ON so.sample_id = spstatus.sample_id
        INNER JOIN orders test_order_order2 ON test_order_order2.order_id=so.order_id
        WHERE test_order_order2.order_id = test_order_order.order_id AND spstatus.status = 'AUTHORIZED'
