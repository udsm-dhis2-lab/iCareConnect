...
--       (SELECT GROUP_CONCAT(DISTINCT CASE WHEN results_name.name IS NULL THEN CONCAT(testalloc_name.name,'-',ob.value_text) ELSE CONCAT('[',testalloc_name.name,'-',results_name.name,']') END)
       (SELECT GROUP_CONCAT(DISTINCT CASE WHEN results_name.name IS NULL AND test_result.value_text IS NOT NULL THEN test_result.value_text WHEN results_name.name IS NULL AND test_result.value_numeric IS NOT NULL AND testalloc_name.name = "HIV - Viral Load" THEN test_result.value_numeric WHEN  ELSE results_name.name END)
		-- 	FROM lb_sample_order so
-- 			INNER JOIN lb_test_allocation testalloc ON so.order_id = testalloc.order_id
-- 			INNER JOIN lb_test_result test_result ON test_result.test_allocation_id = testalloc.test_allocation_id
...
--       (SELECT GROUP_CONCAT(DISTINCT CASE WHEN results_name.name IS NULL THEN CONCAT(testalloc_name.name,'-',ob.value_text) ELSE CONCAT('[',testalloc_name.name,'-',results_name.name,']') END)
       (SELECT
          SUBSTRING_INDEX(
            ...
            DISTINCT
            CASE
            WHEN results_name.name IS NULL AND test_result.value_text IS NOT NULL THEN test_result.value_text
            WHEN results_name.name IS NULL AND test_result.value_numeric IS NOT NULL AND testalloc_name.name = "HIV - Viral Load" THEN test_result.value_numeric
            ELSE results_name.name END
            ORDER BY test_result.test_result_id DESC
            SEPARATOR ','
            ),
            ',',
            1
