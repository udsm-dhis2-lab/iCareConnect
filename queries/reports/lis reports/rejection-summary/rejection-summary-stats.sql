...
FROM(
 SELECT
		...
		WHERE v.voided=0 AND CAST(CONVERT_TZ(v.date_started,'Etc/GMT+3','GMT') AS DATE) BETWEEN "2022-01-01" AND "2023-12-31" AND l.name IS NOT NULL
...
-- 			INNER JOIN orders test_order_order ON test_order_order.encounter_id=test_order_encounter.encounter_id
-- 			INNER JOIN concept test_order_concept ON test_order_concept.concept_id=test_order_order.concept_id
--             INNER JOIN concept_name test_order_concept_name2 ON test_order_concept_name2.concept_id=test_order_concept.concept_id AND test_order_concept_name2.concept_name_type='FULLY_SPECIFIED'
--             INNER JOIN visit_attribute va ON va.visit_id = v.visit_id
-- 			LEFT JOIN location lo on lo.uuid = va.value_reference
--             WHERE test_order_concept_name2.name = test_order_concept_name.name AND lo.location_id = l.location_id
--
--         )AS "number_of_requests",
--         (SELECT SUM(IF(test_order_concept.concept_id =219907, 1, 0))
--
...
-- 		WHERE v.voided=0 AND CAST(CONVERT_TZ(v.date_started,'Etc/GMT+3','GMT') AS DATE) BETWEEN "2022-01-01" AND "2023-12-31" AND l.name IS NULL
-- 		GROUP BY l.location_id,test
        )	AS A
