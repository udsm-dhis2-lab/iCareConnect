SELECT (@row_number:=@row_number+1) AS "NA",`JINA_LA_MGONJWA`,`TAREHE_YA_KULAZWA`,`JINSIA_YA_MGONJWA`,`WODI` FROM(
SELECT

	UPPER(CONCAT(pn.given_name,' ',pn.family_name)) AS `JINA_LA_MGONJWA`,
	GROUP_CONCAT(DISTINCT DATE_FORMAT(visit_encounter.encounter_datetime, "%d/%m/%Y %h:%i %p")) AS `TAREHE_YA_KULAZWA`,
