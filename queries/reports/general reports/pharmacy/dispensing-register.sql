SELECT (@row_number:=@row_number+1) AS "Na",TAREHE,`JINA LA MGONJWA`,`DAWA`,`IDADI`,`UNIT` FROM(
SELECT
		...
		GROUP_CONCAT(DISTINCT d.name) AS `DAWA`,
        GROUP_CONCAT(do.quantity) AS `IDADI`,
        GROUP_CONCAT(unit.name) AS `UNIT`
	...
	WHERE  v.voided=0 AND CAST(CONVERT_TZ(v.date_started,'Etc/GMT+3','GMT') AS DATE) BETWEEN "2023-03-15" AND "2023-03-30"
