...
//public class LaboratoryControllerBKP {
//
//	@Autowired
...
@Controller
@RequestMapping(value = { "/rest/" + RestConstants.VERSION_1 + "/lab", "/ws/rest/" + RestConstants.VERSION_1 + "/lab" })
public class LaboratoryController {
	...
	public Map<String, Object> getStorageTypes(HttpServletRequest request,
		...
		List<Map<String, Object>> storageTypeObjects = new ArrayList<Map<String, Object>>();
		for (StorageType storageType : results.getResults()) {
			storageTypeObjects.add(storageType.toMap());
		}
		Map<String, Object> response = new HashMap<String, Object>();
		response.put("pager", buildPagerMap(request, pager));
		response.put("storageTypes", storageTypeObjects);
		return response;
	}
	...
	@ResponseBody
	public ResponseEntity<Map<String, Object>> deleteStorageType(@PathVariable("storageTypeUuid") String storageTypeUuid,
	...
	public Map<String, Object> getStorages(HttpServletRequest request,
		...
		List<Map<String, Object>> storageObjects = new ArrayList<Map<String, Object>>();
		for (Storage storage : results.getResults()) {
			storageObjects.add(storage.toMap());
		}
		Map<String, Object> response = new HashMap<String, Object>();
		response.put("pager", buildPagerMap(request, pager));
		response.put("storages", storageObjects);
		return response;
	}
	...
	@ResponseBody
	public ResponseEntity<Map<String, Object>> deleteStorage(@PathVariable("storageUuid") String storageUuid,
	        @RequestParam(value = "reason", required = false) String reason) {
		try {
	...
	private HttpStatus resolveStorageExceptionStatus(Exception exception) {
		...
		if (message.contains("already exists") || message.contains("still associated") || message.contains("still in use")
		        || message.contains("cannot be deleted") || message.contains("remove or reassign")) {
	...
	private String resolveStorageExceptionCode(HttpStatus status, Exception exception) {
		...
		if (status == HttpStatus.CONFLICT) {
			if (message.contains("storage type") && (message.contains("still") || message.contains("cannot be deleted"))) {
				return "STORAGE_TYPE_IN_USE";
			}
			if (message.contains("storage type")) {
				return "STORAGE_TYPE_CONFLICT";
			}
