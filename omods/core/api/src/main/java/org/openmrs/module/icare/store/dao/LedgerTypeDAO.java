package org.openmrs.module.icare.store.dao;

// Generated Oct 7, 2020 12:49:21 PM by Hibernate Tools 5.2.10.Final

import org.openmrs.module.icare.core.dao.BaseDAO;
import org.openmrs.module.icare.store.models.LedgerType;
import org.springframework.stereotype.Repository;

/**
 * Home object for domain model class StLedgerType.
 * 
 * @see org.openmrs.module.icare.core.dao.StLedgerType
 * @author Hibernate Tools
 */

public class LedgerTypeDAO extends BaseDAO<LedgerType> {
	
}
