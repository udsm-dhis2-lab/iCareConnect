...
public class Sample extends BaseOpenmrsData implements java.io.Serializable, JSONConverter {
	...
	public Map<String, Object> toMap() throws Exception {
		...
		List<Map<String, Object>> sampleStatusesList = new ArrayList<Map<String, Object>>();
		SampleStatus latestStorageStatus = null;
		SampleStatus latestDisposalStatus = null;

		for (SampleStatus sampleStatus : this.getSampleStatuses()) {

			sampleStatusesList.add(sampleStatus.toMap());
			if (sampleStatus.getRetired() != null && sampleStatus.getRetired()) {
				continue;
			}
			if (isStorageLifecycleStatus(sampleStatus) && isLaterLifecycleStatus(sampleStatus, latestStorageStatus)) {
				latestStorageStatus = sampleStatus;
			}
			if (isDisposalLifecycleStatus(sampleStatus) && isLaterLifecycleStatus(sampleStatus, latestDisposalStatus)) {
				latestDisposalStatus = sampleStatus;
			}
		}

		sampleObject.put("statuses", sampleStatusesList);
		if (latestDisposalStatus != null) {
			Map<String, Object> disposalObject = getStructuredLifecycleDetails(latestDisposalStatus);
			disposalObject.put("category", latestDisposalStatus.getCategory());
			disposalObject.put("status", latestDisposalStatus.getStatus());
			disposalObject.put("timestamp", latestDisposalStatus.getTimestamp());
			sampleObject.put("disposed", true);
			sampleObject.put("disposal", disposalObject);
			sampleObject.put("currentStorage", null);
		} else {
			Map<String, Object> currentStorageObject = null;
			if (latestStorageStatus != null && !isStorageRemovalStatus(latestStorageStatus)) {
				currentStorageObject = getStructuredLifecycleDetails(latestStorageStatus);
				currentStorageObject.put("category", latestStorageStatus.getCategory());
				currentStorageObject.put("status", latestStorageStatus.getStatus());
				currentStorageObject.put("timestamp", latestStorageStatus.getTimestamp());
			}
			sampleObject.put("disposed", false);
			sampleObject.put("disposal", null);
			sampleObject.put("currentStorage", currentStorageObject);
		}
...
//		patientObject.put("phone", phoneNumber);
	...
	public Map<String, Object> toMap(Object... params) throws Exception {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("dd-M-yyyy hh:mm:ss");
		boolean includeOrderEncounter = (params.length > 0 && params[0] != null) ? SafeTypeCast.orDefault(params[0], Boolean.class, false) : false;
		...
		for (SampleOrder sampleOrder : this.getSampleOrders()) {
			...
			if (!sampleOrder.getOrder().getVoided()) {
				orders.add(sampleOrder.toMap(false, includeOrderEncounter));
			}
		}
...
//		patientObject.put("phone", phoneNumber);
	...
	private boolean isStorageLifecycleStatus(SampleStatus sampleStatus) {
		String category = normalizeLifecycleCategory(sampleStatus != null ? sampleStatus.getCategory() : null);
		return category.startsWith("STORAGE");
	}
	
	private boolean isDisposalLifecycleStatus(SampleStatus sampleStatus) {
		String category = normalizeLifecycleCategory(sampleStatus != null ? sampleStatus.getCategory() : null);
		return category.startsWith("DISPOSE");
	}
	
	private boolean isStorageRemovalStatus(SampleStatus sampleStatus) {
		String category = normalizeLifecycleCategory(sampleStatus != null ? sampleStatus.getCategory() : null);
		return "STORAGE_REMOVED".equals(category) || "STORAGE_RETRIEVED".equals(category)
		        || "STORAGE_RELEASED".equals(category);
	}
	
	private boolean isLaterLifecycleStatus(SampleStatus candidate, SampleStatus currentLatest) {
		if (candidate == null) {
			return false;
		}
		...
		Long candidateTime = candidate.getTimestamp() != null ? candidate.getTimestamp().getTime() : 0L;
		Long latestTime = currentLatest.getTimestamp() != null ? currentLatest.getTimestamp().getTime() : 0L;
		return candidateTime >= latestTime;
	}
	
	private String normalizeLifecycleCategory(String category) {
		return category == null ? "" : category.trim().toUpperCase();
	}
	
	private Map<String, Object> getStructuredLifecycleDetails(SampleStatus sampleStatus) {
		Map<String, Object> details = new LinkedHashMap<String, Object>();
		if (sampleStatus == null) {
			return details;
		}
		if (sampleStatus.getRemarks() == null || sampleStatus.getRemarks().trim().equals("")) {
			return details;
		}
		String[] tokens = sampleStatus.getRemarks().split(";");
		...
		if (!hasStructuredContent) {
			details.put("rawRemarks", sampleStatus.getRemarks());
		}
		return details;
	}
