	...
	private class OrderBillCreationAdvice implements MethodInterceptor {
		...
		public Object invoke(MethodInvocation invocation) throws Throwable {
			...
			if (invocation.getArguments()[0] instanceof TestOrder) {
				...
				ItemPrice itemPrice = Context.getService(ICareService.class).getItemPriceByConceptAndVisit(
						order.getEncounter().getVisit(),
						order.getConcept());
				if (itemPrice == null) {
				...
				if (((DrugOrder) invocation.getArguments()[0]).getDrug() != null) {
					...
					if (order.getAction() == Order.Action.NEW) {
						 ...
						 * administrationService.getGlobalProperty(ICareConfig.STOCK_LOCATIONS);
						 * if (stockLocations == null) {
						 * throw new
						 * ConfigurationException("Stock Locations is configured. Please set '"
						...
						if (order.getQuantity() != null && order.getQuantity() != 0) {
							...
							for (Location location : locationService
									.getLocationsByTag(locationService.getLocationTagByName("Dispensing Unit"))) {
								locationUuids.add(location.getUuid());
							}
							// List<Stock> stockList =
							// storeService.getStockByItemAndLocation(item.getUuid(),
							// order.getEncounter().getLocation().getUuid());
							...
							if (!(stockEnabled != null && stockEnabled.equals("false"))) {
								List<Stock> stockList = storeService.getStockByItemAndLocations(item.getUuid(),
										locationUuids);
								if (stockList.size() == 0) {
						...
						StoreService storeService = Context.getService(StoreService.class);
						List<Stock> stockList = storeService.getStockByItemAndLocation(item.getUuid(),
								order.getEncounter()
										.getLocation().getUuid());
						if (stockList.size() == 0) {
							throw new StockOutException(item.getDisplayString() + " is stocked out.");
				...
				if (order.getDrug() != null) {
					...
					Prescription prescriptionOrder = (Prescription) Context.getOrderService()
							.getOrderByUuid(order.getUuid());
					// List<OrderStatus> prescriptionOrderStatuses =
					// Context.getService(StoreService.class).getOrderStatusByOrderUuid(prescriptionOrder.getUuid());
					if (!orderMetaData.getItemPrice().getPaymentType().getName().getName().toLowerCase()
							.equals("cash")) {
				...
				AdministrationService administrationService = Context.getAdministrationService();
				String orderTypeToSkipBilling = administrationService
						.getGlobalProperty(ICareConfig.ORDER_TO_SKIP_BILLING_ADVISOR);
