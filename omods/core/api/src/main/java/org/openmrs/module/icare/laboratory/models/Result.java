...
public class Result extends BaseOpenmrsData implements java.io.Serializable {
	...
	public static Result fromMap(Map<String, Object> map) throws ParseException {
		...
		if ((map.get("valueNumeric")) != null) {
			String value = map.get("valueNumeric").toString();
			try {
				result.setValueNumeric(Double.valueOf(value));
			} catch (NumberFormatException e) {
				System.err.println("Invalid number format for valueNumeric: " + value);
				result.setValueNumeric(null);
			}
		}
		...
		if (map.get("associatedFieldResult") != null) {
			...
			associatedField.setUuid(
					((Map) ((Map) map.get("associatedFieldResult")).get("associatedField")).get("uuid").toString());
		...
		if (map.get("testedDate") != null) {
			...
			if (map.get("testedDate").toString().length() == 10) {
				...
				result.setTestedDate(dateFormat.parse(
						map.get("testedDate").toString().substring(0, map.get("testedDate").toString().indexOf("T"))));
			}
		}
