...
public class LaboratoryServiceImpl extends BaseOpenmrsService implements LaboratoryService {
		...
		for (StorageLocationType candidate : getAllStorageLocationTypesUnsafe()) {
			...
			if (Boolean.TRUE.equals(candidate.getSlotBearing())) {
				...
				if (name.equalsIgnoreCase("position") || name.equalsIgnoreCase("slot") || name.equalsIgnoreCase("well")
						|| name.equalsIgnoreCase("cell") || code.equalsIgnoreCase("POSITION") || code.equalsIgnoreCase("WELL")
						|| code.equalsIgnoreCase("CELL")) {
	...
	private String generateSlotLabel(int layer, int row, int column, String pattern) {
		...
		String label = defaultPattern
				.replace("${layer}", String.valueOf(layer)).replace("{layer}", String.valueOf(layer))
				.replace("${row}", rowLabel).replace("{row}", rowLabel)
				.replace("${rowNumber}", String.valueOf(row)).replace("{rowNumber}", String.valueOf(row))
				.replace("${column}", String.valueOf(column)).replace("{column}", String.valueOf(column));
