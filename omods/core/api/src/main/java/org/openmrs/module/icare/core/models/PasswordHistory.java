...
public class PasswordHistory implements Serializable {
    ...
    public static PasswordHistory fromMap(Map<String, Object> passwordHistoryMap) throws Exception {
        ...
        if(passwordHistoryMap.get("user") != null){
            User user = Context.getUserService().getUserByUuid(((Map)passwordHistoryMap.get("user")).get("uuid").toString());
        ...
        if(passwordHistoryMap.get("date") != null){
            ...
            if (passwordHistoryMap.get("date").toString().length() == 10) {
                ...
                passwordHistory.setChangedDate(dateFormat.parse(passwordHistoryMap.get("date").toString().substring(0, passwordHistoryMap.get("date").toString().indexOf("T"))));
            }
        }
