...
export class BmiComponent implements OnInit {
  ...
  constructor(
    ...
    this.bmi$ = zip(this.observations$, this.heightConcept$, this.weightConcept$).pipe(map((res) => {
      let observations = res[0];
      let heightUuid = res[1];
      let weightUuid = res[2];
      return  Math.round((observations[weightUuid]?.latest?.value/(observations[heightUuid]?.latest?.value * 0.01 * observations[heightUuid]?.latest?.value * 0.01)*10))/10;
    }))
  }
