...
export class OtherPatientDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin([this.patientVisit$, this.allpatientVisit$]).subscribe(([patientVisit, allpatientVisit]) => {
      // Handle patientVisit$
      ...
      const insuranceIDs = allpatientVisit.map(visit => {
        const insuranceAttribute = visit.visit.attributes.find(attribute => attribute.display.includes("Insurance ID"));
        return insuranceAttribute ? insuranceAttribute.value : null;
      });
      console.log("insuranceAttribute  ---------------------------------------===:", insuranceIDs);
     
      const validInsuranceIDs = insuranceIDs.filter(id => id !== null).map(id => parseInt(id, 10));
